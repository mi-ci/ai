...
def reset() :
    ...
    cv.putText(img, 'e:erase s:show r:recognition q:quit', (10,40), cv.FONT_HERSHEY_COMPLEX, 0.8, (255,0,0), 1)
...
def recognition() :
    ...
    numerals = numerals.astype(np.float32)/255
    res = model.predict(numerals)
    class_id = np.argmax(res, axis=1)
    for i in range(5) :
        cv.putText(img, str(class_id[i]), (50+i*100, 180), cv.FONT_HERSHEY_COMPLEX, 1, (255,0,0), 1)
